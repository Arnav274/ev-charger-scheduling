"""Unit tests for DijkstraStrategy in app.algorithms."""
...
def _make_station(lat: float, lon: float) -> MagicMock:
    """Return a mock that satisfies the Station interface used by algorithms."""
    s = MagicMock()
    s.id = uuid.uuid4()
    s.lat = lat
    s.lon = lon
...
def test_score_is_haversine_distance():
    """DijkstraStrategy score must equal the straight-line haversine distance."""
...
def test_score_increases_with_distance():
    """Stations placed further from the origin must receive higher scores."""
...
def test_score_ignores_travel_by_station():
    """DijkstraStrategy must use its own graph computation, not OSRM travel_by_station."""
    station = _make_station(lat=1.0, lon=0.0)
    ctx_no_osrm = _make_context(0.0, 0.0)

    ctx_with_osrm = _make_context(0.0, 0.0)
    # Inject a wildly wrong OSRM distance — Dijkstra score must be unaffected.
...
def test_three_stations_ranked_by_dijkstra_distance():
    """Sorting three stations by their Dijkstra score must yield nearest-first order."""
    # Place stations at ~1.1 km, ~5.5 km, ~11.1 km due north of the origin.
